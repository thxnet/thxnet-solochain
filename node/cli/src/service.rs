...
pub fn create_extrinsic(
	...
	function: impl Into<thxnet_runtime::RuntimeCall>,
	nonce: Option<u32>,
	...
	let period = thxnet_runtime::BlockHashCount::get()
		.checked_next_power_of_two()
		.map(|c| c / 2)
		.unwrap_or(2) as u64;
	let tip = 0;
	let extra: thxnet_runtime::SignedExtra = (
		frame_system::CheckNonZeroSender::<thxnet_runtime::Runtime>::new(),
		frame_system::CheckSpecVersion::<thxnet_runtime::Runtime>::new(),
		frame_system::CheckTxVersion::<thxnet_runtime::Runtime>::new(),
		frame_system::CheckGenesis::<thxnet_runtime::Runtime>::new(),
		frame_system::CheckEra::<thxnet_runtime::Runtime>::from(generic::Era::mortal(
			period,
			best_block.saturated_into(),
		)),
		frame_system::CheckNonce::<thxnet_runtime::Runtime>::from(nonce),
		frame_system::CheckWeight::<thxnet_runtime::Runtime>::new(),
		pallet_asset_tx_payment::ChargeAssetTxPayment::<thxnet_runtime::Runtime>::from(
			tip, None,
		),
	);

	let raw_payload = thxnet_runtime::SignedPayload::from_raw(
		function.clone(),
		extra.clone(),
		(
			(),
			thxnet_runtime::VERSION.spec_version,
			thxnet_runtime::VERSION.transaction_version,
			genesis_hash,
			best_hash,
			(),
			(),
	...
	thxnet_runtime::UncheckedExtrinsic::new_signed(
		function,
		sp_runtime::AccountId32::from(sender.public()).into(),
		thxnet_runtime::Signature::Sr25519(signature),
		extra,
	)
}
